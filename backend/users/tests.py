...
def test_user_list_requires_admin(client):
    ...
    User.objects.create_user(
        email="vol@example.com", name="Vol", password="volpass", role="VOLUNTEER"
    )
    ...
    res_forbidden = client.get(
        "/api/users/list/", HTTP_AUTHORIZATION=f"Bearer {access_vol}"
    )
...
def test_user_list_filters(client):
    ...
    User.objects.create_user(
        email="a1@example.com", name="A1", password="x", role="ADMIN", is_active=True
    )
    User.objects.create_user(
    ...
    # Role filter
    res_role = client.get(
        "/api/users/list/?role=VOLUNTEER", HTTP_AUTHORIZATION=f"Bearer {token}"
    )
    ...
    res_role = client.get(
        "/api/users/list/?role=ADMIN", HTTP_AUTHORIZATION=f"Bearer {token}"
    )
    assert res_role.status_code == status.HTTP_200_OK
    data = res_role.json()
    results = data["results"] if isinstance(data, dict) and "results" in data else data
    ...
    res_active = client.get(
        "/api/users/list/?is_active=true", HTTP_AUTHORIZATION=f"Bearer {token}"
    )
    ...
    res_inactive = client.get(
        "/api/users/list/?is_active=false", HTTP_AUTHORIZATION=f"Bearer {token}"
    )
    ...
    res_invalid = client.get(
        "/api/users/list/?role=INVALID_ROLE", HTTP_AUTHORIZATION=f"Bearer {token}"
    )
...
def test_user_update_is_active_and_expiry(client):
    ...
    User.objects.create_user(
        email="admin2@example.com", name="Admin2", password="adminpass", role="ADMIN"
    )
    target = User.objects.create_user(
        email="target@example.com", name="Target", password="pwd", role="VOLUNTEER"
    )
...
def test_user_update_requires_admin(client):
    ...
    User.objects.create_user(
        email="admin3@example.com", name="Admin3", password="adminpass", role="ADMIN"
    )
    User.objects.create_user(
        email="vol2@example.com", name="Vol2", password="volpass", role="VOLUNTEER"
    )
    target = User.objects.create_user(
        email="target2@example.com", name="Target2", password="pwd", role="VOLUNTEER"
    )
...
def test_admin_cannot_update_self(client):
    ...
    admin = User.objects.create_user(
        email="admin4@example.com", name="Admin4", password="adminpass", role="ADMIN"
    )
